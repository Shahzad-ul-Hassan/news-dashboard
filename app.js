/* Dashboard frontend (no build tools). Loads JSON from /data and /archive. */
const $ = (id) => document.getElementById(id);

function escapeHtml(s){ return (s ?? "").toString().replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c])); }
function fmtNumber(n,d=2){ if(n===null||n===undefined||Number.isNaN(Number(n))) return "—"; return Number(n).toLocaleString(undefined,{maximumFractionDigits:d}); }
function fmtPct(n,d=2){ if(n===null||n===undefined||Number.isNaN(Number(n))) return "—"; const v=Number(n); const sign=v>0?"+":""; return `${sign}${v.toFixed(d)}%`; }
function clsForChange(v){ if(v===null||v===undefined||Number.isNaN(Number(v))) return "mid"; if(v>0) return "good"; if(v<0) return "bad"; return "mid"; }
function arrowForTrend(tr){ if(tr==="bullish") return {a:"↑",cls:"good"}; if(tr==="bearish") return {a:"↓",cls:"bad"}; return {a:"→",cls:"mid"}; }
...
const state = {
  ...
  config:null, i18n:{en:{}, ur:{}},
  markets:null, news:null, signals:null, sessions:null,
  watchTab:"crypto",
  filters:{category:"all", impact:"all", q:"", date:""},
  feedCursor:0, feedPageSize:25,
  selectedNewsId:null
};
...
function buildTimingTile(name,tz,openHM,closeHM){
  ...
  if(name==="CRYPTO"){ status="open"; label="Live 24/7"; }
  else if(weekend){ status="closed"; label="Weekend"; }
  else if(now.asUTC<openUTC){ status="pre_open"; label="Opens in"; count=msToHMS(openUTC-now.asUTC); }
  else if(now.asUTC<closeUTC){ status="open"; label="Closes in"; count=msToHMS(closeUTC-now.asUTC); }
  else { status="closed"; label="Opens in"; count=msToHMS(openUTC+86400000-now.asUTC); }

  const st = status==="open"?t("market_open"):(status==="pre_open"?t("market_preopen"):t("market_closed"));
  const timeStr=`${now.parts.hour}:${now.parts.minute}:${now.parts.second}`;
...
function renderTimings(){
  ...
  $("timings").innerHTML = [
    buildTimingTile("US", ms.US.tz, ms.US.open, ms.US.close),
    buildTimingTile("PSX", ms.PSX.tz, ms.PSX.open, ms.PSX.close),
...
function renderRatesStrip(){
  ...
  const wanted=["BTCUSDT","ETHUSDT","^GSPC","^NDX","USD/PKR","EUR/USD"];
  const picked=wanted.map(w=>items.find(x=>x.symbol===w)).filter(Boolean);
  const final=picked.length?picked:items.slice(0,6);
  $("ratesStrip").innerHTML = final.map(it=>{
    ...
    return `<a class="rate-chip" href="${escapeHtml(it.source_url||"#")}" target="_blank" rel="noreferrer">
      ...
      <span class="rate-price">${escapeHtml(fmtNumber(it.price,4))}</span>
...
function renderWatchlist(){
  ...
  $("wlHint").textContent = (state.lang==="ur") ? "نوٹ: فری ڈیٹا میں کچھ مارکیٹس میں تاخیر ہو سکتی ہے۔" : "Note: Free data may be delayed for some markets.";
    ...
    return `<div class="card">
      <div class="row">
        ...
        <div style="text-align:right">
          <div class="mono">${escapeHtml(fmtNumber(it.price,4))}</div>
  ...
  const html = slice.map(it=>{
    ...
    return `<div class="card" id="card_${escapeHtml(it.id)}" style="${selected?"outline:2px solid rgba(255,255,255,.18)":""}">
      ...
      <div class="links" style="margin-top:10px">
        ...
        ${(it.linked_assets||[]).slice(0,6).map(a=>`<span class="badge">${escapeHtml(a)}</span>`).join("")}
      </div>
    </div>`;
  }).join("");
...
function renderSignals(){
  ...
  $("signalsActive").innerHTML = active.slice(0,5).map(sig=>{
  ...
  $("signalsClosed").innerHTML = closed.slice(0,10).map(sig=>{
...
function selectNews(newsId, scrollTo=false){
  ...
  if(el){ el.style.outline="2px solid rgba(255,255,255,.18)"; if(scrollTo) el.scrollIntoView({behavior:"smooth",block:"start"}); }
  if(state.signals){
    const a=state.signals.active||[];
    state.signals = {...state.signals, active:[...a.filter(s=>s.news_id===newsId), ...a.filter(s=>s.news_id!==newsId)]};
    renderSignals();
  }
...
function bindFilters(){
  ...
  $("btnClearFilters").onclick = async ()=>{ 
    ...
    $("filterCategory").value="all"; $("filterImpact").value="all"; $("filterDate").value=""; $("globalSearch").value="";
    $("modeBadge").hidden=true;
    await loadLatest();
  };
  $("filterDate").onchange = async (e)=>{
    const val=e.target.value; state.filters.date=val;
...
function setLang(lang){
  ...
  renderTimings(); renderRatesStrip(); renderWatchlist(); renderSignals(); renderSessions(); renderNewsFeed(true);
}
...
async function loadLatest(){
  try{
    ...
    state.config=cfg; state.markets=mk; state.news=nw; state.signals=sg; state.sessions=ss;
    $("lastUpdated").textContent = (state.lang==="ur"?"آخری اپڈیٹ: ":"Last updated: ") + ((nw.generated_at_utc||mk.generated_at_utc||"").replace("T"," ").replace("Z"," UTC"));
    renderTimings(); renderRatesStrip(); renderWatchlist(); renderSignals(); renderSessions(); renderNewsFeed(true);
  }catch(e){ console.error(e); toast(state.lang==="ur"?"ڈیٹا لوڈ نہیں ہو سکا۔":"Failed to load data."); }
}
async function loadArchiveDate(ymd){
  ...
  }catch(e){ console.error(e); toast(state.lang==="ur"?"اس تاریخ کی فائل موجود نہیں۔":"No archive file for this date."); }
}
...
async function init(){
  ...
  $("btnCollapseLeft").onclick = ()=>{ 
    ...
    if(el.style.display==="none"){ el.style.display=""; toast(state.lang==="ur"?"واچ لسٹ دکھا دی گئی۔":"Watchlist shown."); }
    else { el.style.display="none"; toast(state.lang==="ur"?"واچ لسٹ چھپا دی گئی۔":"Watchlist hidden."); }
  };
